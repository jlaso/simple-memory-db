...
$databases['countries'] = new CountryTable(file_get_contents(__DIR__.'/countries.json'));
//var_dump($databases['taxes']->findAll()); die;
...
foreach($databases['customers']->findAll('country_id', $countryId) as $customer){
    ...
    printf("Customer (%d) %s applies %s '%s' [%.2f%%]\r\n", $customer['id'], $customer['name'], $tax['name'], $tax['subtype'], $tax['value']);
}
